import { Cpu, BellRing, Radio, ShieldCheck, TrendingUp, TrendingDown, Minus } from 'lucide-react'
...
export function KpiCards() {
  ...
  const msgRate = data.messages_per_minute
  const msgLabel = msgRate >= 1000 ? `${(msgRate / 1000).toFixed(1)}K` : String(msgRate)
  ...
  return (
    <div className="kpi-grid">
      {[
        ...
        { label: 'Mensagens/min', value: msgLabel, icon: Radio, iconClass: 'info', trend: msgRate > 0 ? 'up' : 'neutral', trendLabel: msgRate > 0 ? `+${msgRate}` : 'Aguardando dados', footer: 'MQTT + OPC-UA', barPct: Math.min(100, msgRate), barColor: 'var(--info)' },
